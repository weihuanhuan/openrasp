...
public class JSContextFactory extends ContextFactory {
    ...
    public static boolean setCheckScriptList(List<CheckScript> checkScriptList) {
    ...
    public static void setCloudCheckScript(String plugin, String md5, String version, Long deliveryTime) {
        if (jsContextFactory != null) {
            try {
                ...
                Config.commonLRUCache.clear();
                //插件更新成功后，设置algorithmConfig
                algorithmConfigSet(cx, tempScope);
            } catch (Throwable e) {
                String message = "new plugin update failed";
                LOGGER.warn(CloudUtils.getExceptionObject(ErrorType.PLUGIN_ERROR, message), e);
            } finally {
    ...
    private static ScriptableObject perpareLoadPlugin(JSContext cx, ScriptableObject scope) throws Exception {
        ...
        JSPrecompiledLoader loader = new JSPrecompiledLoader(cx, globalScope);
        loader.loadPrecompiled();
...
//        cx.evaluateString(scope, script, name, 1, null);
     ...
     * 插件更新成功后，设置algorithmConfig
     */
    private static void algorithmConfigSet(Context context, ScriptableObject scope) throws Exception {
        if (RASP != null) {
            Object config = RASP.get("algorithmConfig");
            if (config != null) {
                Object stringify = NativeJSON.stringify(context, scope, config, null, null);
                Config.getConfig().setConfig(ConfigItem.ALGORITHM_CONFIG, stringify, true);
            }
        }
    }
}
